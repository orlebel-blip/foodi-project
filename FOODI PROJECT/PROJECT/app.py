...
def hungarian_best_restaurants(
    ...
    """
    בחירת מסעדות באמצעות אלגוריתם הונגרי
    """

    if not restaurants:
        return []

    # מספר "לקוחות" וירטואליים
    num_clients = min(top_k, len(restaurants))
    num_restaurants = len(restaurants)

    # מטריצת עלות: לקוחות × מסעדות
    cost_matrix = np.zeros((num_clients, num_restaurants))

    for i in range(num_clients):              # לקוחות
        for j, r in enumerate(restaurants):   # מסעדות
            cost_matrix[i, j] = (
